...
<!-- For the vignette, we will consider a subset of taxa and paired Asteraceae plants.  -->
...
ps <- subset_samples(ps, sample_names(ps) %in% paired_aster)
select_taxa <- names(sort(taxa_sums(ps), decreasing = TRUE)[1:50])
ps <- prune_taxa(select_taxa, ps)
```
